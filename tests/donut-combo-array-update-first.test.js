...
describe('first positional operator', () => {
    ...
    it('can change the color of the first white donut found in every document', async () => {
        await seeder.runMix2(db);

        const initialDonutCombos = await donutCombosCollection.find({ }).toArray();

        /** @type {import('mongodb').UpdateWriteOpResult} */
        const result = await donutCombosCollection.updateMany({ 'donuts.color': 'white' }, { 
            $set: { 
                'donuts.$.color': 'pink'
            } 
        });
        ...
        expect(modifiedDonutCombos[0].donuts[0].color).toBe('pink');
        expect(modifiedDonutCombos[0].donuts[1].color).toBe('white');
        expect(modifiedDonutCombos[1].donuts[1].color).toBe('pink');

        await logger.printDifference(expect.getState().currentTestName, initialDonutCombos, modifiedDonutCombos);
    });

    it('can remove the color property of the first white donut found in every document', async () => {
        await seeder.runMix2(db);
